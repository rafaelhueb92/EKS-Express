...
resource "aws_iam_policy" "karpenter_policy" {
  ...
  description = "IAM policy for Karpenter operations"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
      ...
      },
      {
        ...
        Effect   = "Allow"
        Action   = "iam:PassRole"
        Resource = "arn:aws:iam::${local.account_id}:role/KarpenterNodeRole-${local.eks_cluster_name}"
      },
      {
        Sid      = "EKSClusterEndpointLookup"
        Effect   = "Allow"
        Action   = "eks:DescribeCluster"
        Resource = "arn:aws:eks:${var.region}:${local.account_id}:cluster/${local.eks_cluster_name}"
      },
      {
